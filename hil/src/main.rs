...
fn run_cmd(args: &[&str], cwd: &Path) -> Result<String> {
    ...
    cmd.args(&args[1..]);
    cmd.current_dir(cwd);
    let output = cmd.output().with_context(|| format!("Failed to execute command: {:?}", args))?;
    ...
    if engine_path.exists() && engine_path.join(".git").exists() {
        if let Ok(current_commit) = run_cmd(&["git", "rev-parse", "HEAD"], engine_path) {
            if current_commit.trim() == lock.commit {
                println!("[hil] engine/ is already initialized at commit {}.", lock.commit);
                return Ok(());
            }
        }
    }
...
fn setup_git(commit: &str, engine_path: &Path, repo_root: &Path) -> Result<()> {
    ...
    let local_ff = repo_root.join("firefox");
    if local_ff.exists() && local_ff.join(".git").exists() {
        println!("[hil] Found local firefox/ clone, cloning locally (offline-friendly)...");
        let output = Command::new("git")
            .args(&["clone", "--shared", "firefox", "engine"])
            .current_dir(repo_root)
            .output()?;
        if !output.status.success() {
            bail!("Failed to clone locally: {}", String::from_utf8_lossy(&output.stderr));
        }
    } else {
        ...
        let output = Command::new("git")
            ...
            .output()?;
        if !output.status.success() {
            bail!("Failed to clone Firefox repository: {}", String::from_utf8_lossy(&output.stderr));
        }
    }
    
    // Set local git config for committing
    run_cmd(&["git", "config", "user.name", "Hilal Tool"], engine_path)?;
    run_cmd(&["git", "config", "user.email", "hil-tool@hilal.browser"], engine_path)?;
    ...
    let output = Command::new("git")
        ...
        .output()?;
    if !output.status.success() {
        bail!("Failed to checkout commit: {}", String::from_utf8_lossy(&output.stderr));
    }
...
fn apply(repo_root: &Path, engine_path: &Path, force: bool, dry_run: bool) -> Result<()> {
    ...
    if run_cmd(&["git", "rev-parse", "upstream-base"], engine_path).is_err() {
        let _ = run_cmd(&["git", "tag", "-f", "upstream-base", &lock.commit], engine_path);
    }
    ...
    for entry in &manifest.patches {
        ...
        if !src.exists() {
            bail!("Path declared in manifest does not exist: changes/{}", entry.path);
        }
        ...
        if entry.path.ends_with(".patch") {
            ...
            if !dry_run {
                ...
                Command::new("git")
                    .args(&["commit", "--allow-empty", "-m", &format!("Apply patch: {}", entry.path)])
                    .current_dir(engine_path)
                    .output()?;
            }
            applied += 1;
            ...
            if !dry_run {
                ...
                run_cmd(&["git", "add", "-A"], engine_path)?;
                Command::new("git")
                    .args(&["commit", "--allow-empty", "-m", &format!("Sync overlay: {}", entry.path)])
                    .current_dir(engine_path)
                    .output()?;
            }
            copied += 1;
...
fn download_ublock(engine_path: &Path) -> Result<()> {
    ...
    if ubo_path.exists() {
        ...
        if current_sha == ubo_sha256 {
            println!("[hil] uBlock Origin v{} is already present and verified.", ubo_version);
            return Ok(());
        }
    }
    
    println!("[hil] Downloading uBlock Origin v{} extension...", ubo_version);
    ...
    if !curl_output.status.success() {
        ...
        bail!("Failed to download uBlock via curl: status code {}, error: {}", curl_output.status, err);
    }
    ...
    println!("[hil] uBlock Origin v{} successfully downloaded and verified.", ubo_version);
    Ok(())
}
...
fn refresh(repo_root: &Path, engine_path: &Path) -> Result<()> {
    ...
    let commits_str = run_cmd(&["git", "rev-list", "--reverse", "upstream-base..HEAD"], engine_path)?;
    ...
    for entry in &manifest.patches {
        ...
        if entry.path.ends_with(".patch") {
            if commit_index >= commits.len() {
                println!("[hil] Warning: No commit found corresponding to patch changes/{}", entry.path);
                continue;
            }
...
fn verify(repo_root: &Path) -> Result<()> {
    ...
    if current_sha == lock.tarball_sha256 {
        ...
        bail!("CRITICAL: Tarball checksum mismatch! Expected {}, got {}", lock.tarball_sha256, current_sha);
    }
    Ok(())
}
